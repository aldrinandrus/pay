...
## Demo: typescript/site — setup & troubleshooting

If you're testing the Next.js demo site (the one that protects a weather report behind x402), follow these steps and checks. These tips address the most common causes of a persistent `402 Payment Required` after you believe you've paid on-chain.

- Create a `.env.local` in `typescript/site` with these values (example):

```env
# Exact network string must match a supported network (the demo defaults to `base-sepolia`)
NETWORK=base-sepolia

# The address that should receive funds (resource server wallet)
RESOURCE_WALLET_ADDRESS=0xYourResourceWalletAddress

# Optional: the facilitator URL used by the client-side paywall (defaults to /api/facilitator)
NEXT_PUBLIC_FACILITATOR_URL=/api/facilitator

# Optional client-side API key used by some UI helpers
NEXT_PUBLIC_ONCHAINKIT_API_KEY=

# Required for settle (if you use the example facilitator settle endpoint)
PRIVATE_KEY=0xYourFacilitatorPrivateKey
```
...
- Reproduce the flow:
  1. In a browser, open the protected route (the demo shows a protected weather report). The resource should initially return `402` with payment requirements. Save the `paymentRequirements` block if needed.
  2. Use your client wallet (the example client runs locally under `examples/typescript/clients/*`) to perform the payment according to the `paymentRequirements` (send the correct token on the correct chain to the `payTo` address).
  3. After sending the on-chain transfer, the client will call the facilitator verify endpoint. If verification succeeds the page will unlock.

What to look for in logs if it still stays `402`:

- Server console (Next.js server) will now print debug lines for verify/settle requests. Look for lines starting with `[x402][verify]` and `[x402][settle]`.
  - `[x402][verify] incoming verify request` — shows which `network` the client requested and whether a `paymentPayload` was present.
  - If you see `payTo mismatch` warnings the client paid to a different address than your configured `RESOURCE_WALLET_ADDRESS` (common mistake). Confirm the on-chain transfer's `to` address matches `RESOURCE_WALLET_ADDRESS` in `typescript/site/.env.local`.

- If verify fails because of `invalid_network`, double-check the `NETWORK` string (it must match one of the supported network names exactly — the demo uses `base-sepolia`).

- If the client called `/settle` and it failed due to missing keys, check `PRIVATE_KEY` is set (used by the example facilitator to submit a settlement transaction). If you prefer not to settle on-chain, verify should still succeed when the on-chain transfer is already present.

What to paste here when asking for more help:

- The Next.js server console output for the `[x402][verify]` and `[x402][settle]` logs when you trigger a payment.
- The on-chain transaction hash (tx hash) of your USDC transfer, and the token contract address used (so I can confirm the token and recipient matched the `paymentRequirements.asset` and `payTo`).

Notes about a recent runtime improvement

- I added a small runtime sanity check to the demo `verify` endpoint: it compares the `paymentRequirements.payTo` value the client selected against the demo's configured `RESOURCE_WALLET_ADDRESS`. If they don't match, verification returns a clear `invalidReason: "invalid_exact_evm_payload_recipient_mismatch"` and logs a warning. This makes mis-sent payments (wrong recipient address) much easier to diagnose.

If you run the demo and still see `402`, collect the server logs and the transaction hash and paste them here — I'll analyze and implement any further fixes needed.
