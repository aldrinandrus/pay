...
2. Configure your environment variables in `.env` or `.env.local` (use `.env.local` for local-only settings):

  ```bash
  # the URL where your facilitator API is hosted (defaults to https://x402.org/facilitator)
  NEXT_PUBLIC_FACILITATOR_URL=your_facilitator_url

  # the wallet address that should receive payments; must match the `payTo` used by the client
  RESOURCE_WALLET_ADDRESS=your_wallet_address

  # the network to use for payments (e.g. base-sepolia, base, polygon)
  NETWORK=base-sepolia

  # (optional) private key for settlement transactions
  PRIVATE_KEY=your_private_key

  # (optional, dev-only) skip on-chain verification, auto-approve payments
  SKIP_X402_VERIFY=true
  ```

  > **Troubleshooting tips:**
  > - If you experience intermittent 402 errors, make sure `RESOURCE_WALLET_ADDRESS` is set and matches the address shown in the paywall. The server now falls back to the `payTo` value if the variable is missing.
  > - The library retries verification once, so a short delay between payment and retry may resolve transient failures. You can still watch logs for `invalidReason` values.
  > - Check the server console for logs starting with `[x402][verify]` or `[x402-next]`—they print the decoded payment payload, requirements, and invalid reason.
  > - You can temporarily enable `SKIP_X402_VERIFY` to bypass verification while investigating issues.
...
> Remember to check the terminal for `[x402]` log lines when exercising the payment flow — they help explain why a verify/settle call succeeded or failed.
