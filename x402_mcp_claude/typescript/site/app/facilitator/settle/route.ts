...
export async function POST(req: Request) {
  ...
  // Debug logs to help diagnose settlement failures (network mismatch, missing key, wrong receiver, etc.)
  // eslint-disable-next-line no-console
  console.log('[x402][settle] incoming settle request', {
    network,
  ...
  if (!privateKey) {
    // eslint-disable-next-line no-console
    console.error('[x402][settle] missing private key for network', network);
