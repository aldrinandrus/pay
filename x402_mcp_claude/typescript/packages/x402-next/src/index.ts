...
export function paymentMiddleware(
  ...
  return async function middleware(request: NextRequest) {
    ...
    if (!selectedPaymentRequirements) {
      console.warn("[x402-next] no matching payment requirements for decoded payload", {
        decodedPayment,
        paymentRequirements,
      });
      return new NextResponse(
    ...
    // Development bypass: if SKIP_X402_VERIFY is set to "true", treat every payment as valid
    if (process.env.SKIP_X402_VERIFY === "true") {
      console.warn("[x402-next] SKIP_X402_VERIFY enabled, skipping verification and auto-approving payment");
    } else {
      // log decoded payment and selected requirements to help track intermittent mismatches
      console.debug("[x402-next] decodedPayment", decodedPayment);
      console.debug("[x402-next] selectedPaymentRequirements", selectedPaymentRequirements);

      // helper that retries a few times for transient failures
      async function verifyWithRetry(attempts = 2) {
        let last;
        for (let i = 0; i < attempts; i++) {
          console.debug("[x402-next] verify attempt", i + 1);
          const result = await verify(decodedPayment, selectedPaymentRequirements);
          console.debug("[x402-next] verify result", result);
          if (result.isValid) return result;
          last = result;
          if (
            (result.invalidReason === 'invalid_payment' ||
              result.invalidReason === 'unexpected_verify_error') &&
            i + 1 < attempts
          ) {
            console.warn("[x402-next] transient verify failure, retrying in 1s");
      ...
      const verification = await verifyWithRetry(2);

      if (!verification?.isValid) {
        console.warn("[x402-next] verification failed", verification);
        return new NextResponse(
          JSON.stringify({
            x402Version,
